...
const geniusLyricsAPI = process.env.GENIUS;
...
module.exports = class LyricsCommand extends Command {
  ...
  async run(message, { songName }) {
    if (
      ...
      console.log("Running lyric command on " + songName );
    ...
    // get song id
    songName = songName.replace(/ *\([^)]*\) */g, ''); // remove stuff like (feat. Juice WRLD)
    songName = songName.replace(/ *\[[^)]*\] */g, ''); // remove stuff like [Official Audio]
    console.log("Stripped name is " + songName );
    ...
    try {
      ...
      console.log(result.response.hits[0].result);
      // get lyrics
...
//      console.log("Url: " + url );
      ...
      if (lyrics.length > 4095) {
        console.log("Lyrics longer than max of 4095: " + lyrics.length);
        console.log("Dumping first 2048 characters for inspection: " + lyrics.slice(0,2048));
        return message.say(
